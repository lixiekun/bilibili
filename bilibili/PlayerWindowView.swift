...
struct PlayerWindowView: View {
    ...
    @State private var isHidingForImmersive = false // 进入沉浸模式时隐藏窗口层但不卸载播放器
    @State private var isPlayerReady = false        // 新增：用于追踪播放器是否真正就绪
        ...
        ZStack(alignment: .topLeading) {
            Group {
                if let player = playerModel.player {
                    // 沉浸模式下完全移除 PlayerControllerView，避免黑屏残留
                    // 并且在沉浸模式下保持 PlayerWindowView 挂载，防止 ImmersiveSpace 被连带关闭
                    if playerModel.presentation == .inline {
            ...
            // 沉浸模式下不显示关闭按钮
            // 移除自定义关闭按钮，完全依赖 AVPlayerViewController 原生的返回/关闭行为
            // 这样可以避免与详情页返回按钮重叠的问题
        }
        .task {
            await playerModel.loadVideo(playInfo: playInfo, cid: cid, bvid: bvid)
            // 加载完成后（loadVideo 是 async 的），延迟一小会儿再显示关闭按钮，
            // 避免和系统自带的关闭/返回动画冲突，或者在白屏/黑屏期间显示出来
            try? await Task.sleep(for: .milliseconds(500))
            withAnimation {
                isPlayerReady = true
            }
        }
        .onChange(of: playerModel.presentation) { _, newValue in
            isHidingForImmersive = (newValue == .immersive)
        }
        .onDisappear {
            print("🎬 PlayerWindowView onDisappear. presentation: \(playerModel.presentation), hidingForImmersive: \(isHidingForImmersive)")
            // 如果是进入沉浸模式导致的消失，不清理播放器
            if isHidingForImmersive || playerModel.presentation == .immersive {
                print("🎬 跳过清理，窗口被沉浸模式暂时隐藏")
            } else {
    ...
    // 移除了 enterImmersiveSpace 方法，改为由 ContentView 统一处理
